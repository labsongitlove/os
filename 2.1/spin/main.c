...
static pthread_spinlock_t counters_lock;

static void inc_counter(long *c)
{
    pthread_spin_lock(&counters_lock);
    ++(*c);
    pthread_spin_unlock(&counters_lock);
}
...
int main(int argc, char **argv)
{
    ...
    pthread_spin_init(&counters_lock, PTHREAD_PROCESS_PRIVATE);
    ...
    for (;;) {
        sleep(2);
        pthread_spin_lock(&counters_lock);
        printf("[spinlock] iters: inc=%ld dec=%ld eq=%ld | swaps=%ld\n",
